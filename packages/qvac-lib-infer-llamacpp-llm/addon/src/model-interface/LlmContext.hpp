...
public:
  ...
  void setCompactionPolicy(
      std::shared_ptr<qvac_lib_inference_addon_llama::runtime::CompactionPolicy>
          policy) {
    compactionPolicy_ = std::move(policy);
  }
  [[nodiscard]] qvac_lib_inference_addon_llama::runtime::CompactionPolicy&
  compactionPolicy() {
    return *compactionPolicy_;
  }
  [[nodiscard]]
  const qvac_lib_inference_addon_llama::runtime::CompactionPolicy&
  compactionPolicy() const {
    return *compactionPolicy_;
  }
